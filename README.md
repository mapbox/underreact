...
This could come in handy when you want to do some quick inspection/editting of unminified files.
...
Running a production mode with the `start` command gives you the ability to live reload your changes. This could come in handy when you are trying to fix a bug which doesn't show up in development environment.
...
There are many cases, for example [Jest](#how-do-i-make-jest-use-underreacts-babel-configuration-), that expect `babel.config.js` to exist at the root your project. In this case it is best to create a `babel.config.js` at the root of your project:
...
The above polyfills should allow you to freely use `for of loops`, `async functions`, or `spread operator` in your code.

If your application needs any other polyfill, say `fetch`, you can install it and import it at the top of your [`jsEntry`](#jsentry) file:
...
**Tip:** If you are used to using `NODE_ENV` to target different deployment environments, you should instead use `DEPLOY_ENV` because of the reasons cited above and Underreact makes no assumption about the value of `DEPLOY_ENV`.
...
There is no guarantee that the node module you use would work well with the browsers you are planning to target. This is why **Underreact compiles all of the node modules to standard ES5 Javascript by default.**
...
Set it to `false` to prevent automatic refreshing of your app on code changes. Switching off `liveReload` would also disable [hot reloading](#hot).
...
Path to the base directory on the domain where the site will be deployed. The default value is root.
...
Pass a directory to which Webpack would write stats. By default, no stats file would be generated.
...
- **Selectively compile node modules.** By either selectively compiling or disabling [compilation of node modules](#compilenodemodules) you can save some compilation time and also reduce build size.
