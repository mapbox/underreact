// Note: this is copied from https://github.com/facebook/create-react-app/blob/79485d7d9ce9fe63d9203c02b663b82fdecf7ac5/packages/react-dev-utils/ModuleNotFoundPlugin.js#L10
// The README advises to copy the code snippet directly as the plugins
// development is aligned with `react-script` and can introduce unexpected changes.
'use strict';

const chalk = require('chalk');
const path = require('path');

// This plugin improves upon the default Webpack module not found error.
module.exports = class ModuleNotFoundPlugin {
  constructor(appPath) {
  ...
  prettierError(err) {
    ...
    if (request) {
      ...
      if (isModule) {
        ...
        details = [`Cannot find file '${chalk.bold(target)}' in '${context}'.`];
      } else {
        details = [err.message];
      }
    } else {
    ...
    err.message = [`There was a problem in ${file}`, ...details]
      .join('\n')
      .replace('Error: ', '');
