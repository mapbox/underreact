...
const webpackPluginsList = require('./plugins-list');
const webpackLoadersList = require('./loaders-list');
  ...
  const config = {
    ...
    output: {
      ...
      filename: urc.isProductionMode
        ? path.join(urc.publicAssetsPath, '/js/[name]-[chunkhash:10].js')
        : path.join(urc.publicAssetsPath, '/js/[name].js'),
      chunkFilename: urc.isProductionMode
        ? path.join(urc.publicAssetsPath, '/js/[name]-[chunkhash:10].chunk.js')
        : path.join(urc.publicAssetsPath, '/js/[name].chunk.js')
    },
    performance: false,
    plugins: webpackPluginsList(urc),
    resolve: {
      alias: {
