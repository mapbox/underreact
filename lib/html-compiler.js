...
function renderHtml(urc, cssFilename) {
  ...
  let html;
  try {
    if (path.extname(urc.htmlSource) === '.html') {
      html = fs.readFileSync(urc.htmlSource, 'utf8');
    } else {
    ...
    throw new Error(
      `Failed to read HTML from ${path.relative(process.cwd(), urc.htmlSource)}`
    );
  }
...
function writeHtml(urc, cssFilename) {
  ...
  const html = renderHtml(urc, cssFilename);
  fs.writeFileSync(path.join(urc.outputDirectory, 'index.html'), html);
}
